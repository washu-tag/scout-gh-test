...
for ((i = 0; i <= max_wait; ++i)); do
    if sudo kubectl exec -n temporal -it service/temporal-admintools -- temporal workflow list -o json | jq '[.[] | select(.taskQueue == "ingest-hl7-log")] | any(.[]; .status == "WORKFLOW_EXECUTION_STATUS_COMPLETED")' -e > /dev/null; then
